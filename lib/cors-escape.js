...
function proxyRequest(req, res, proxy) {
    ...
    if (!location.hostname.match(/^([a-z0-9-]+(-[a-z0-9]+)*\.)+[a-z]{2,}$/)) {
        ...
        location.href = `https://my.farm.bot/` + `${location.href}`.replace('http://', '')
        location.protocol = "https:"
        location.host = 'my.farm.bot'
        location.hostname = 'my.farm.bot'
    }
...
function getHandler(options, proxy) {
    ...
    return function (req, res) {
        ...
        if (!location.hostname.match(/^([a-z0-9-]+(-[a-z0-9]+)*\.)+[a-z]{2,}$/)) {
            location.pathname = `${location.hostname}/${location.pathname}`.replace(/\/\//g, '/')
            location.path = `${location.hostname}/${location.path}`.replace(/\/\//g, '/')
            location.href = `http://my.farm.bot/` + `${location.href}`.replace('http://', '')
            location.protocol = "https:"
            location.host = 'my.farm.bot'
            location.hostname = 'my.farm.bot'
        }
